...
export class ActionComponent implements DoCheck, OnChanges {
	...
	static DEFAULT_FORMAT = { add: 'Add', remove: 'Remove', all: 'All', none: 'None', direction: ActionComponent.LTR, draggable: true };

	@Input() id = `dual-list-${nextId++}`;
	@Input() key = '_id';
	@Input() display = '_name';
	@Input() height = '100px';
	@Input() format = ActionComponent.DEFAULT_FORMAT;
  ...
  @Input() presenter:any;
	...
	constructor(private differs:IterableDiffers) {
		this.available = new BasicList(ActionComponent.AVAILABLE_LIST_NAME);
		this.confirmed = new BasicList(ActionComponent.CONFIRMED_LIST_NAME);
	}
	...
	ngOnChanges(changeRecord: {[key:string]:SimpleChange}) {
		...
		if (changeRecord['format']) {
			...
			if (typeof(this.format.direction) === 'undefined') {
				this.format.direction = ActionComponent.LTR;
			}

			if (typeof(this.format.add) === 'undefined') {
				this.format.add = ActionComponent.DEFAULT_FORMAT.add;
			}

			if (typeof(this.format.remove) === 'undefined') {
				this.format.remove = ActionComponent.DEFAULT_FORMAT.remove;
			}

			if (typeof(this.format.all) === 'undefined') {
				this.format.all = ActionComponent.DEFAULT_FORMAT.all;
			}

			if (typeof(this.format.none) === 'undefined') {
				this.format.none = ActionComponent.DEFAULT_FORMAT.none;
			}

			if (typeof(this.format.draggable) === 'undefined') {
				this.format.draggable = ActionComponent.DEFAULT_FORMAT.draggable;
			}
		}
	...
	buildAvailable(source:Array<any>) : boolean {
		...
		if (sourceChanges) {
			...
			this.available.shift = this.available.list;
	...
	buildConfirmed(destination:Array<any>) : boolean {
		...
		if (destChanges) {
			...
			this.confirmed.shift = this.confirmed.list;
	...
	direction() {
		return this.format.direction === ActionComponent.LTR;
	}
	...
	allowDrop(event:DragEvent, list:BasicList) {
		if (event.dataTransfer && event.dataTransfer.types.length && (event.dataTransfer.types[0] === this.id)) {
	...
	shiftClick(event:MouseEvent, index:number, source:BasicList, item:any) {
		if (event.shiftKey && source.last && !Object.is(item, source.last)) {
			const idx = source.shift.indexOf(source.last);
			if (index > idx) {
				for (let i = (idx + 1); i < index; i += 1) {
					this.selectItem(source.pick, source.shift[i]);
				}
			} else if (idx !== -1) {
	...
	selectAll(source:BasicList,actionType:string) {
		source.pick.length = 0;
		source.pick = source.shift.slice(0);
    if(actionType === 'add') {
      this.moveItem(this.available, this.confirmed)
    } else {
      this.moveItem(this.confirmed, this.available)
	...
	private unpick(source:BasicList) {
		for (let i = source.pick.length - 1; i >= 0; i -= 1) {
			if (source.shift.indexOf(source.pick[i]) === -1) {
	...
	onFilter(source:BasicList) {
		if (source.picker.length > 0) {
			...
			source.shift = filtered;
			this.unpick(source);
		} else {
			source.shift = source.list;
		}
	}
